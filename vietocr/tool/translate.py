...
def beamsearch(memory, model, device, beam_size=2, candidates=1, max_seq_length=128, sos_token=1, eos_token=2):    
    # memory: Tx1xE
    model.eval()
...
def batch_process_image(images, files, image_height, image_min_width, image_max_width, batch_size):
    cluster_imgs = defaultdict(list)
    ...
    for i, image in enumerate(images):
        ...
        img = np.asarray(img).transpose(2,0, 1)
        img = img/255
        cluster_imgs[new_w].append(img)
        cluster_files[new_w].append(files[i])
    
    batch_lists = []
    batch_files = []
    for cluster, cluster_img in cluster_imgs.items():
        batches = [cluster_img[i:i + batch_size] for i in range(0, len(cluster_img), batch_size)]
        for batch in batches:
            batch = np.asarray(batch)
            batch = torch.FloatTensor(batch)
            batch_lists.append(batch)

        batch_files += cluster_files[cluster]

    return batch_lists, batch_files
